	...
	public class NodeCollection : IEnumerable
	{
		...
		public Node slowGetNodeFromVisibleIndex(int index)
		{
			...
			while (iterator.MoveNext())
			{
				...
				if (index < 0)
				{
                    TreeList.Tracing.EndTrack(0, "slowGetNodeFromVisibleIndex({0})", startindex);
					return iterator.Current as Node;
				}
			}
            TreeList.Tracing.EndTrack(0, "slowGetNodeFromVisibleIndex (null)");
			return null;
		}
